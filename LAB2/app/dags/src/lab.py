...
def load_data():
    ...
    """
    print("We are here")
    df = pd.read_csv('/opt/airflow/dags/data/file.csv')
...
def build_save_model(data_b64: str, filename: str):
    """
    Builds a Gaussian Mixture Model on the preprocessed data and saves it.
    Returns the BIC list (JSON-serializable) for elbow method.
    """
    # decode -> bytes -> numpy array
    data_bytes = base64.b64decode(data_b64)
    df = pickle.loads(data_bytes)
    
    gmm_kwargs = {"covariance_type": "full", "random_state": 42, "max_iter": 300}
    bic = []
    ...
    return bic  # list is JSON-safe

def load_model_elbow(filename: str, bic: list):
    """
    Loads the saved GMM model and uses the elbow method to report optimal k.
    ...
    # elbow for information/logging (BIC: lower is better)
    kl = KneeLocator(range(1, 50), bic, curve="convex", direction="decreasing")
    print(f"Optimal no. of clusters: {kl.elbow}")
    
    # predict on raw test data (GMM uses predict() just like KMeans)
    df = pd.read_csv('/opt/airflow/dags/data/test.csv')
